...
| **Revues gratuites par jour** (ajouté le 23/08, retour testeur) | `resultat_revues_gratuites_par_jour`, défaut 1 | Indépendant du quota d'examens : sur plusieurs examens gratuits passés le même jour, seuls les premiers (à hauteur de cette valeur) donnent une revue gratuite — les suivants retombent sur `resultat_verrouille`. Compteur du jour par quizz (`EtatQuizz.revuesAujourdhui`), remis à zéro comme les autres quotas. |
